...
<section class="project-card">
	<div class="picture-wrapper" bind:this={imageContainer}>
		...
		<picture bind:this={pictureElement}>
			<source
				type="image/avif"
				srcset={`${PUBLIC_ASSETS_URL}/${project.cover_image}?format=avif&width=300&quality=80&fit=cover 300w, ${PUBLIC_ASSETS_URL}/${project.cover_image}?format=avif&width=800&quality=80&fit=cover 800w`}
				sizes="(max-width: 800px) 100vw, 50vw"
			/>
			<img
				src={`${PUBLIC_ASSETS_URL}/${project.cover_image}?format=jpg&width=800&quality=80&fit=cover`}
				loading="lazy"
				alt={project.title}
			/>
		</picture>
